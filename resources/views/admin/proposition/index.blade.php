        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            ...
            <div class="mb-8 flex flex-col sm:flex-row justify-between items-center gap-4">
                <nav class="flex space-x-2 bg-white p-1.5 rounded-2xl shadow-sm border border-slate-100 max-w-fit w-full sm:w-auto">
                    <a href="{{ route('admin.propositions.index', ['statut' => 'en_attente', 'annee' => $annee]) }}" class="{{ $status === 'en_attente' ? 'bg-indigo-50 text-indigo-700' : 'text-slate-500 hover:text-slate-700 hover:bg-slate-50' }} rounded-xl px-5 py-2.5 text-sm font-semibold transition duration-300">
                        En attente
                    </a>
                    <a href="{{ route('admin.propositions.index', ['statut' => 'approuve', 'annee' => $annee]) }}" class="{{ $status === 'approuve' ? 'bg-emerald-50 text-emerald-700' : 'text-slate-500 hover:text-slate-700 hover:bg-slate-50' }} rounded-xl px-5 py-2.5 text-sm font-semibold transition duration-300">
                        Approuvées
                    </a>
                    <a href="{{ route('admin.propositions.index', ['statut' => 'rejete', 'annee' => $annee]) }}" class="{{ $status === 'rejete' ? 'bg-rose-50 text-rose-700' : 'text-slate-500 hover:text-slate-700 hover:bg-slate-50' }} rounded-xl px-5 py-2.5 text-sm font-semibold transition duration-300">
                        Rejetées
                    </a>
                </nav>
                ...
                <div class="flex items-center gap-3 w-full sm:w-auto">
                    ...
                    <form method="GET" action="{{ route('admin.propositions.index') }}" class="flex items-center" id="filter-form-props">
                        ...
                        <select name="annee" onchange="document.getElementById('filter-form-props').submit()" class="rounded-xl border-slate-200 text-sm focus:border-indigo-500 focus:ring-indigo-500 shadow-sm text-slate-600 bg-white">
                    ...
                    <!-- Export Buttons -->
                    <a href="{{ route('admin.propositions.export.excel', ['statut' => $status, 'annee' => $annee]) }}" class="inline-flex items-center gap-1.5 px-3 py-2 bg-emerald-50 text-emerald-700 hover:bg-emerald-100 rounded-xl text-sm font-semibold transition">
                        <svg class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 10v6m0 0l-3-3m3 3l3-3m2 8H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z"/></svg>
                        Excel
                    </a>
                    <a href="{{ route('admin.propositions.export.pdf', ['statut' => $status, 'annee' => $annee]) }}" class="inline-flex items-center gap-1.5 px-3 py-2 bg-rose-50 text-rose-700 hover:bg-rose-100 rounded-xl text-sm font-semibold transition" target="_blank">
