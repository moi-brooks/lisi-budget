<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 leading-tight">
            Nouvelle Ligne Budgétaire — {{ $budget->saison }}
        </h2>
    </x-slot>
    ...
    <div class="py-12">
        <div class="max-w-2xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white shadow-sm rounded-2xl border border-slate-100 p-8">
                ...
                <form method="POST" action="{{ route('admin.budgets.lignes.store', $budget) }}" class="space-y-6">
                    @csrf

                    <div class="grid grid-cols-3 gap-4">
                        <div class="space-y-1">
                            <label class="block text-xs font-bold uppercase tracking-widest text-slate-500">Article</label>
                            <input type="number" name="article" value="{{ old('article') }}" required
                        ...
                        <div class="space-y-1">
                            ...
                            <input type="number" name="paragraphe" value="{{ old('paragraphe') }}" required
                                class="w-full border border-slate-200 rounded-xl px-4 py-3 text-sm focus:ring-2 focus:ring-indigo-500 focus:border-transparent">
                        </div>
                        <div class="space-y-1">
                            <label class="block text-xs font-bold uppercase tracking-widest text-slate-500">Rubrique</label>
                            <input type="number" name="rubrique" value="{{ old('rubrique') }}" required
                                class="w-full border border-slate-200 rounded-xl px-4 py-3 text-sm focus:ring-2 focus:ring-indigo-500 focus:border-transparent">
                        </div>
                    </div>
                    ...
                    <div class="space-y-1">
                        ...
                        <input type="text" name="code_ligne" value="{{ old('code_ligne') }}" required
